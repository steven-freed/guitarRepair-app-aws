...
@property (nonatomic, strong) AWSFormTableCell *nameRow;
...
- (void)setUp {
    _nameRow = [[AWSFormTableCell alloc] initWithPlaceHolder:@"Name" type:InputTypeText];
    ...
    [self.tableDelegate addCell:self.nameRow];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([@"SignUpConfirmSegue" isEqualToString:segue.identifier]){
        ...
        NSString *emailCon = [self.tableDelegate getValueForCell:self.emailRow forTableView:self.tableView];
        signUpConfirmationViewController.user = [self.pool getUser:emailCon];
    }
}
...
- (IBAction)onSignUpClicked:(id)sender {
    ...
    AWSCognitoIdentityUserAttributeType * nameAtt = [AWSCognitoIdentityUserAttributeType new];
    nameAtt.name = @"name";
    nameAtt.value = [self.tableDelegate getValueForCell:self.nameRow forTableView:self.tableView];
    AWSCognitoIdentityUserAttributeType * phoneAtt = [AWSCognitoIdentityUserAttributeType new];
    phoneAtt.name = @"phone_number";
    phoneAtt.value = [self.tableDelegate getValueForCell:self.phoneNumberRow forTableView:self.tableView];
    AWSCognitoIdentityUserAttributeType * emailAtt = [AWSCognitoIdentityUserAttributeType new];
    emailAtt.name = @"email";
    emailAtt.value = [self.tableDelegate getValueForCell:self.emailRow forTableView:self.tableView];
    
    if(![@"" isEqualToString:phoneAtt.value]){
        [attributes addObject:phoneAtt];
    }
    if(![@"" isEqualToString:emailAtt.value]){
        [attributes addObject:emailAtt];
    }
    if(![@"" isEqualToString:nameAtt.value]){
        [attributes addObject:nameAtt];
    }
    
    NSString *email = [self.tableDelegate getValueForCell:self.emailRow forTableView:self.tableView];
